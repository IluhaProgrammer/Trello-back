...
export class AuthService {
    ...
    async register(dto: AuthDto) {
        ...
        if (oldUser) throw new BadRequestException('Пользователь с таким email уже зарегистрирован')

        const {password, ...user} = await this.user.create(dto)
    ...
    async getNewTokens(refreshToken : string) {
        ...
        const {password, ...user} = await this.user.getById(result.id)
